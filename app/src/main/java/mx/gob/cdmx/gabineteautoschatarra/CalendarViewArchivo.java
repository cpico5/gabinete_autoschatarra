...
// builder.setMessage(cuentaArchivos())
	builder.setMessage(
		"Total de Registros:\t\t\t" + dameTotal(fecha)+"\n\n"+
		"Completos:\t\t\t" + dameNormal(fecha)+"\n\n"+
...
// Abrimos la base de datos 'DBUsuarios' en modo escritura
	...
	String cuantosNormal;
	db = usdbh.getWritableDatabase();
	String selectQuery1 = "select count(*) as total from encuestas where tipo_captura='COMPLETA' and  fecha='" + fecha + "' and usuario<>'1'";
	Cursor c = db.rawQuery(selectQuery1, null);
	if (c.getCount() > 0) {
// Recorremos el cursor hasta que no haya más registros
		c.moveToFirst();
